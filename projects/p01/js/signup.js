...
function isValidEmail(email) {
    ...
    if (emailRegex.test(input.value.trim())){
        fnc_ShowSuccess(input);
    }
    else
    {
        fnc_ShowErr(input, `${getFeildId(input)} is not a valid username`);
        //return !!email && typeof email === 'string' && email.match(emailformat)
    }
}
...
function checkrequired(inputArray){
  // loop required in inputArray
  inputArray.forEach(function(input) {
      if (input.value === '') 
      {
          //fnc_ShowErr(input, input.id + ' is required');
          fnc_ShowErr(input, `${getFeildId(input)} is required `);
      }
      else
      {
          fnc_ShowSuccess(input);
...
function checklength(input, minlength, maxlength)
{
    if (input.value.length < minlength){
        fnc_ShowErr(input, `${getFeildId(input)} needs to be atleast ${minlength} characters `);
    }
    else if (input.value.length < minlength){
        fnc_ShowErr(input, `${getFeildId(input)} needs to less than ${maxlength} characters `);
    }
    else
    {
        fnc_ShowSuccess(input);
...
form.addEventListener('submit', function(e){
    ...
    checkrequired([fname,lname,emailaddr,username,password,cpassword]);
